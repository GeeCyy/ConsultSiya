...
function buildReportHtml(sections) {
  ...
  const pagesHtml = sections.map(({ professor, rows }, idx) => {
    const tableRows = rows.map((row, i) => {
      ...
      // For file-type proofs, prefer a signed Cloudinary delivery URL (opens directly,
      // no login needed) so the exported link never shows as empty/missing; legacy
      // local-disk uploads fall back to our own serving endpoint.
      let proofUrl = null;

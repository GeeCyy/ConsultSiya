...
router.post('/', authenticate, authorize('admin'), async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `INSERT INTO announcements (title, body, type, pinned)
       VALUES ($1, $2, $3, $4)
       RETURNING id, title, body, type, pinned, created_at`,
    ...
    const result = await pool.query(
      `UPDATE announcements
       SET title = COALESCE($2, title),
           ...
           updated_at = NOW()
       WHERE id = $1
       RETURNING id, title, body, type, pinned, created_at, updated_at`,
      [id, title || null, body || null, type || null, pinned ?? null]
    );

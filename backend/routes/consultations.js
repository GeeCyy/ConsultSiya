...
// ── Digital advising slip ─────────────────────────────────────────────────────

// PUT /api/consultations/:id/slip — professor fills the digital slip (outcome, referred_to, notes)
router.put('/:id/slip', authenticate, authorize('professor'), async (req, res) => {
  const { outcome, referred_to, prof_notes } = req.body;
  if (outcome && !['resolved', 'follow_up'].includes(outcome)) {
    return res.status(400).json({ error: 'outcome must be "resolved" or "follow_up".' });
  }
  try {
    ...
    const result = await pool.query(
      `UPDATE consultations
       SET slip_outcome      = $1,
           slip_referred_to  = $2,
           slip_prof_notes   = $3,
           slip_finalized_at = NOW()
       WHERE id = $4
       RETURNING slip_outcome, slip_referred_to, slip_prof_notes, slip_finalized_at`,
      [outcome || null, referred_to || null, prof_notes || null, req.params.id]
    );
    res.json(result.rows[0]);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
// GET /api/consultations/:id/slip — view digital slip (student owner, professor owner, or admin)
router.get('/:id/slip', authenticate, async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT c.id, c.date, c.time, c.nature_of_advising, c.nature_of_advising_specify,
              c.notes, c.mode, c.status,
              c.slip_outcome, c.slip_referred_to, c.slip_prof_notes, c.slip_finalized_at,
              c.student_id, c.professor_id,
              s.full_name  AS student_name,
              s.student_number, s.program, s.year_level,
              p.full_name  AS prof_name,
              u.id         AS prof_user_id
       FROM consultations c
    ...
    if (req.user.role === 'student') {
      const sr = await pool.query('SELECT id FROM students WHERE user_id = $1', [req.user.id]);
      if (!sr.rows.length || sr.rows[0].id !== row.student_id) {
        return res.status(403).json({ error: 'Forbidden.' });
      }
    } else if (req.user.role === 'professor') {
      if (req.user.id !== row.prof_user_id) {
        return res.status(403).json({ error: 'Forbidden.' });
      }
    }

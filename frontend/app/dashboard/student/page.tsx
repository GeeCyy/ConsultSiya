...
export default function StudentDashboard() {
  ...
  // Digital slip view
  const [viewSlipConsult, setViewSlipConsult] = useState<Consultation | null>(null);
  ...
  const openViewSlip = (c: Consultation) => setViewSlipConsult(c);
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className={`rounded-2xl p-6 w-full max-w-md border shadow-2xl ${isDark ? 'bg-[#1a1a2e] border-white/10' : 'bg-white border-gray-200'}`}>
            <div className="flex items-start justify-between mb-4">
              ...
              <button onClick={() => setViewSlipConsult(null)} className={`p-1.5 rounded-lg ${isDark ? 'text-gray-500 hover:text-gray-300 hover:bg-white/5' : 'text-gray-400 hover:text-gray-700 hover:bg-gray-100'}`}>
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
              </button>
            </div>
            <div className="space-y-3">
              {/* Nature of advising */}
              {viewSlipConsult.nature_of_advising && (
                <div className={`rounded-xl px-4 py-3 ${isDark ? 'bg-white/[0.04]' : 'bg-gray-50'}`}>
                  ...
                  {(() => {
                    try { const arr = JSON.parse(viewSlipConsult.nature_of_advising); return (Array.isArray(arr) ? arr : [viewSlipConsult.nature_of_advising]).map((n: string) => <p key={n} className={`text-xs ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>• {n}</p>); }
                    catch { return <p className={`text-xs ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>• {viewSlipConsult.nature_of_advising}</p>; }
                  })()}
                </div>
              )}
              {/* Outcome */}
              <div className={`rounded-xl px-4 py-3 ${isDark ? 'bg-white/[0.04]' : 'bg-gray-50'}`}>
                ...
                {viewSlipConsult.action_taken ? (
                  <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${viewSlipConsult.action_taken === 'Resolved' ? (isDark ? 'bg-emerald-500/15 text-emerald-400' : 'bg-emerald-50 text-emerald-700') : (isDark ? 'bg-amber-500/15 text-amber-400' : 'bg-amber-50 text-amber-700')}`}>
                    {actionLabel(viewSlipConsult.action_taken, viewSlipConsult.referral, viewSlipConsult.referral_specify)}
                  </span>
                ) : <p className={`text-xs ${isDark ? 'text-gray-600' : 'text-gray-400'}`}>Not yet filled by adviser</p>}
              </div>
              {/* Remarks */}
              {viewSlipConsult.remarks && (
                <div className={`rounded-xl px-4 py-3 ${isDark ? 'bg-white/[0.04]' : 'bg-gray-50'}`}>
                  <p className={`text-[10px] font-semibold uppercase tracking-wider mb-1 ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>Adviser Remarks</p>
                  <p className={`text-xs leading-relaxed ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>{viewSlipConsult.remarks}</p>
                </div>
              )}
              <p className={`text-[11px] text-center ${isDark ? 'text-gray-600' : 'text-gray-400'}`}>
                Need to submit a physically signed copy instead?{' '}
                <button
                  onClick={() => triggerUpload(viewSlipConsult.id)}
                  disabled={uploadingId === viewSlipConsult.id}
                ...
                >
                  {uploadingId === viewSlipConsult.id ? 'Uploading…' : 'Upload a scan'}
                </button>
              </p>
              <button onClick={() => setViewSlipConsult(null)} className={`w-full py-2.5 rounded-xl text-sm font-medium mt-2 transition-colors ${isDark ? 'bg-white/5 text-gray-300 hover:bg-white/10' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                Close
              </button>
            </div>
          </div>

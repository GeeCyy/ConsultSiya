...
export default function MatrixCalendar({ consultations, isDark }: MatrixCalendarProps) {
  ...
  const card = isDark ? 'bg-[#0f1c2e] border-blue-900/20' : 'bg-blue-50/40 border-blue-300';
  const bg = isDark ? 'bg-[#0c1322]' : 'bg-blue-50';
  ...
  const borderCls = isDark ? 'border-blue-900/15' : 'border-blue-200';
  ...
  const getCellBg = (items: Consultation[]) => {
    ...
    return isDark ? 'bg-blue-500/5 border-blue-500/15' : 'bg-blue-100/70 border-blue-300';
  };

  return (
    <div className={`rounded-2xl border overflow-hidden ${card}`}>
      {/* ── Header controls ── */}
      <div className={`px-4 py-3 border-b ${borderCls} flex flex-col sm:flex-row sm:items-center gap-3 ${isDark ? 'bg-blue-950/8' : 'bg-blue-100/70'}`}>
        <div className="flex items-center gap-2">
          <button
            onClick={() => setWeekOffset(o => o - 1)}
            className={`w-7 h-7 flex items-center justify-center rounded-lg transition-colors ${isDark ? 'text-blue-300 hover:text-white hover:bg-blue-500/10' : 'text-blue-500 hover:text-blue-700 hover:bg-blue-100'}`}
          >
          ...
          {weekOffset !== 0 && (
            <button
              ...
              className={`ml-2 text-xs font-semibold px-2.5 py-1 rounded-full border transition-colors ${isDark ? 'text-blue-400 border-blue-500/30 hover:bg-blue-500/10' : 'text-blue-600 border-blue-300 hover:bg-blue-100'}`}
            >
              Today
            </button>
          )}
      ...
      {/* ── Legend ── */}
      <div className={`px-4 py-2 border-b ${borderCls} flex items-center gap-4 flex-wrap ${isDark ? 'bg-blue-950/5' : 'bg-blue-100/50'}`}>
        {[
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[640px] table-fixed border-collapse">
          <thead>
            <tr>
              <th className={`w-[88px] px-3 py-2.5 text-left text-[10px] font-semibold uppercase tracking-wide border-b border-r ${borderCls} ${tm} ${isDark ? 'bg-blue-950/8' : 'bg-blue-100/60'}`}>
                Time
              </th>
              {weekDates.map(d => {
                ...
                return (
                  <th
                    ...
                    className={`px-2 py-2.5 text-center text-[11px] font-semibold border-b border-r last:border-r-0 ${borderCls} cursor-pointer select-none transition-all duration-200 ${
                      isAnimating
                        ? isDark ? 'bg-blue-400/20' : 'bg-blue-300/70'
                        : isSelected
                          ? isDark ? 'bg-blue-500/5' : 'bg-blue-200/70'
                          : isDark ? 'hover:bg-blue-500/5' : 'hover:bg-blue-100'
                    } ${
                      isSelected || d.isToday ? 'text-blue-500' : ts
                    }`}
                  >
                    <div className={`transition-transform duration-150 ${isAnimating ? 'scale-110' : 'scale-100'}`}>
                      <div>{d.day}</div>
                      <div className={`text-[10px] font-normal mt-0.5 ${isSelected || d.isToday ? 'text-blue-400' : tm}`}>
                        {d.date.slice(5).replace('-', '/')}
                      </div>
                      {isSelected && (
                        <div className="mt-1 mx-auto w-1 h-1 rounded-full bg-blue-400" />
                      )}
                    </div>
                  </th>
                );
          ...
          <tbody>
            {slots.map((slot, si) => {
              ...
              return (
                <tr key={slot.start} className={isDark ? (si % 2 === 0 ? 'bg-blue-500/[0.02]' : '') : (si % 2 === 0 ? 'bg-blue-100/50' : 'bg-blue-50/40')}>
                      ...
                      <td
                        ...
                        className={`relative px-1.5 py-1.5 border-r border-b last:border-r-0 ${borderCls} align-top min-h-[40px] transition-colors ${
                          ...
                          isDaySelected && items.length === 0
                            ? isDark ? 'bg-blue-500/[0.025]' : 'bg-blue-100/60'
                            : ''
                        }`}
      ...
      >
        <div className={`border-t px-4 pt-4 pb-5 ${isDark ? 'border-blue-900/15' : 'border-blue-200'}`}>
          {/* Panel header */}
          <div className="flex items-start justify-between mb-3">
            ...
            <button
              ...
              className={`w-6 h-6 flex items-center justify-center rounded-lg transition-colors flex-shrink-0 ml-3 ${isDark ? 'text-blue-300 hover:text-white hover:bg-blue-500/10' : 'text-blue-500 hover:text-blue-700 hover:bg-blue-100'}`}
            >
          ...
          {dayConsults.length === 0 ? (
            <div className={`flex flex-col items-center justify-center py-8 rounded-xl border ${isDark ? 'border-blue-900/15 bg-blue-500/[0.02]' : 'border-blue-200 bg-blue-100/50'}`}>
            ...
            <div className={`rounded-xl border overflow-hidden divide-y ${isDark ? 'border-blue-900/15 divide-blue-900/15' : 'border-blue-200 divide-blue-100'}`}>
              {dayConsults.map(c => (
                <div
                  key={c.id}
                  className={`flex items-center gap-3 px-4 py-3 transition-colors ${isDark ? 'bg-[#10192b] hover:bg-blue-500/10' : 'bg-blue-50/30 hover:bg-blue-100/60'}`}
                >
                  {/* Avatar initials */}
                  <div className="w-8 h-8 rounded-full bg-blue-950 border border-blue-900/50 flex items-center justify-center text-blue-300 text-xs font-semibold flex-shrink-0">
      ...
      {selectedCell && (
        ...
        >
          ...
          >
            <div className={`px-4 py-3.5 border-b flex items-center justify-between ${isDark ? 'border-white/10' : 'border-gray-100'}`}>
              <div>
                <p className={`text-sm font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>
                  {new Date(selectedCell.date + 'T12:00:00').toLocaleDateString('en-PH', { weekday: 'long', month: 'short', day: 'numeric' })}
                </p>
                <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>{selectedCell.slotLabel}</p>
              </div>
              <button
                onClick={() => setSelectedCell(null)}
                className={`w-7 h-7 flex items-center justify-center rounded-lg transition-colors ${isDark ? 'text-gray-400 hover:text-white hover:bg-white/10' : 'text-gray-400 hover:text-gray-700 hover:bg-gray-100'}`}
              >
            ...
            <div className={`divide-y ${isDark ? 'divide-white/5' : 'divide-gray-100'}`}>
              {selectedCell.items.map(c => (
                <div key={c.id} className={`px-4 py-3 flex items-start gap-3 transition-colors ${isDark ? 'hover:bg-white/[0.03]' : 'hover:bg-gray-50'}`}>
                  <div className={`w-8 h-8 rounded-full border flex items-center justify-center text-xs font-semibold flex-shrink-0 ${isDark ? 'bg-blue-900/50 border-blue-600/40 text-blue-300' : 'bg-blue-950 border-blue-900/50 text-blue-300'}`}>
                    {c.student_name.split(' ').filter(Boolean).map(n => n[0]).slice(0, 2).join('').toUpperCase()}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>{c.student_name}</p>
                    {c.program && <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>{c.program}</p>}
                    <div className="flex items-center gap-2 mt-1">
                      <span className={`inline-flex items-center gap-1 text-[10px] font-medium px-1.5 py-0.5 rounded-full ${
                        c.status === 'confirmed'   ? (isDark ? 'bg-blue-500/20 text-blue-300' : 'bg-blue-100 text-blue-700') :
                        c.status === 'pending'     ? (isDark ? 'bg-amber-500/20 text-amber-300' : 'bg-amber-100 text-amber-700') :
                        c.status === 'completed'   ? (isDark ? 'bg-emerald-500/20 text-emerald-300' : 'bg-emerald-100 text-emerald-700') :
                  ...
                  <p className={`text-xs font-mono flex-shrink-0 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                    {(() => {
                      const t = c.time || c.time_start;
                      return t ? fmtMins(toMins(t.slice(0, 5))) : '';
                    })()}
                  </p>
                </div>
              ))}

...
export default function DashboardNavbar({
  ...
  return (
    <header className={`sticky top-0 z-40 flex items-center gap-4 px-6 h-14 ${bg}`}>
      {/* Logo */}
      <div className="flex items-center gap-2.5 flex-shrink-0">
        <img src="/consulta-logo.png" alt="Consulta" className="h-14 w-auto object-contain ml-2" />
        <div>
      ...
      {/* Role badge */}
      <span className="flex-shrink-0 inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-semibold border border-[#CC0000]/40 text-[#CC0000]">
        {role.toUpperCase()}
      </span>
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        ...
        <div ref={notifRef} className="relative">
          ...
          {/* Notification dropdown — full when data provided, simple fallback otherwise */}
          {notifOpen && (
            <div className={`absolute right-0 top-full mt-2 w-80 rounded-xl shadow-2xl overflow-hidden z-[60] border ${dropBg}`}>
              {/* Header */}
        ...
        {/* Theme toggle */}
        <button onClick={onToggleTheme} className={iconBtn} title="Toggle theme">
          {isDark ? (
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}>
        ...
        <div ref={dropdownRef} className="relative">
          <button
            ...
            className="w-8 h-8 rounded-full bg-[#7a0000] flex items-center justify-center text-white text-xs font-bold flex-shrink-0 overflow-hidden ring-2 ring-[#CC0000]/20 hover:ring-[#CC0000]/60 transition-all focus:outline-none"
